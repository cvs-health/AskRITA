...
def test_generate_chart_bytes_with_visualizationdata_bar():
    viz = VisualizationData(labels=['Jan', 'Feb'], values=[{'data': [1, 2], 'label': 'S1'}])
...
def test_add_native_pptx_chart_without_dependency_returns_false():
    ...
    ok = add_native_pptx_chart(slide=None, data={'labels': ['A'], 'values': [{'data': [1]}]}, chart_type='bar', x=0, y=0, cx=1, cy=1)
    assert ok is False
