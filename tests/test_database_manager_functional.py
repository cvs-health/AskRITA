...
def _make_mock_config(
    conn_str: str = "postgresql://user:pass@localhost:5432/mydb",
    cache=False,
    max_results=100,
):
    cfg = Mock()
...
def test_init_skips_connection_test():
    with (
        patch(
            "langchain_community.utilities.sql_database.SQLDatabase.from_uri"
        ) as mock_from_uri,
        patch(
...
def test_get_schema_basic():
    with (
        ...
        strategy.enhance_schema = Mock(
            side_effect=lambda schema, cfg: schema + "-enhanced"
        )
...
def test_get_schema_sets_cache_when_enabled():
    with (
        ...
        schema_tool = Mock()
        schema_tool.name = "sql_db_schema"
        schema_tool.invoke.return_value = "RAW"
        list_tool = Mock()
        list_tool.name = "sql_db_list_tables"
        list_tool.invoke.return_value = "users"
...
def test_get_table_names_success_and_exception():
    with (
        ...
        list_tool = Mock()
        list_tool.name = "sql_db_list_tables"
        list_tool.invoke.return_value = "a,b,c"
