...
from askrita.config_manager import ConfigManager, reset_config, get_config
...
class TestConfigManagerWithRealYAML:
    ...
    def test_load_minimal_config(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix=_YAML_SUFFIX, delete=False) as f:
            minimal_config = {
                'database': {
                    'connection_string': _SQLITE_DB_URL
                },
                'llm': {
                    'provider': 'openai',
                    'model': 'gpt-4o',
...
class TestConfigManagerEnvironmentVariables:
    ...
    def test_env_var_substitution(self):
        ...
        try:
            ...
            assert 'test://connection/string' in config.database.connection_string or \
                   '${TEST_DB_CONNECTION}' in config.database.connection_string
        finally:
            os.unlink(config_path)
