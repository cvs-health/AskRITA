    ...
    if visualization == "scatter":
                ...
                "series": [
                    {
                        "name": _GROUP_A,
                        "label": _GROUP_A,
                        "data": [{"x": 10, "y": 100, "id": 0}, {"x": 20, "y": 150, "id": 1}],
                        "marker": {"enabled": True}
                    },
                    {
                        "name": _GROUP_B,
                        "label": _GROUP_B,
                        "data": [{"x": 15, "y": 120, "id": 2}, {"x": 25, "y": 180, "id": 3}],
                        "marker": {"enabled": True}
                    }
                ]
            }
        return {
            "series": [{
                "name": "Test Series",
                "label": "Data Points",
                "data": [{"x": 10, "y": 100, "id": 0}, {"x": 20, "y": 150, "id": 1}, {"x": 30, "y": 200, "id": 2}],
                "marker": {"enabled": True}
            }]
        }
...
def _mock_structured_output(system_prompt, human_prompt, response_model, **kwargs):
    ...
    from askrita.sqlagent.formatters.DataFormatter import (
        DualVisualizationResponse, UniversalChartData, ChartDataset, DataPoint
    )
    ...
    universal_chart = UniversalChartData(
        ...
        title=_TEST_CHART,
        datasets=[
            ChartDataset(label="Test Series", data=[
                DataPoint(value=10), DataPoint(value=20), DataPoint(value=30)
            ])
        ],
        labels=["A", "B", "C"]
    )
...
def mock_config(sample_config_data, monkeypatch):
    ...
    # Mock properties
    from askrita.config_manager import DatabaseConfig, LLMConfig, WorkflowConfig, FrameworkConfig
    ...
    config.get_business_rule.return_value = sample_config_data["business_rules"]["data_validation"]
...
def mock_llm_manager(mock_config, mock_llm):
    ...
    llm_manager.invoke_with_structured_output_direct.side_effect = _mock_structured_output
    ...
    def mock_with_structured_output(response_model, method="json_schema"):
        ...
        def mock_invoke(messages):
            ...
            return mock_llm_manager.invoke_with_structured_output_direct.side_effect(
                "", "", response_model, visualization=viz_type, sample_data=user_content, data=user_content
            )
...
def mock_sql_agent_workflow_class():
    ...
    with patch('askrita.sqlagent.workflows.SQLAgentWorkflow.SQLAgentWorkflow', create=True) as mock_class:
